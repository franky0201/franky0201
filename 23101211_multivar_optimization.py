...
if __name__ == '__main__':
    # Define the cost function and its gradient (Rosenbrock function)
    ...
    x_init = [-1, 1]   # You can try other initial points
    learn_rate = 0.001 # You can try 0.01, 0.005, and 0.0001
    max_iter = 10000   # You can try 100, 1000, and 100000
    min_tol = 1e-6
    ...
    for i in range(max_iter):
        ...
        xp = x
        gradient = fd(x)  # Get the gradient of f at x
        x = x - learn_rate * gradient  # Update x based on the gradient descent rule
        gd_xs.append(x)
